...
/**
 * PnSeedPeers stores a pre-determined list of bitcoin node addresses. 
 * These nodes are highly-active bitcoin nodes.  
 *
 */
public class PnSeedPeers 
	implements PeerDiscovery{

	NetworkParameters networkParameters;
	int pnseedIndex = 0;
	
	public PnSeedPeers(NetworkParameters networkParameters){
	...
	private InetSocketAddress nextPeer() throws UnknownHostException {
		if(pnseedIndex >= pnSeed.length)return null;
		return new InetSocketAddress(convertAddress(pnSeed[pnseedIndex++]),
				networkParameters.port);
	}
	/**
	...
	private InetSocketAddress[] allPeers() throws UnknownHostException  {
		...
		InetSocketAddress[] addresses = new InetSocketAddress[pnSeed.length];
		
		for(int i=0;i<pnSeed.length;++i){
			addresses[i] = new InetSocketAddress(convertAddress(pnSeed[i]),
					networkParameters.port);
		}
		return addresses;
	}
	
	private InetAddress convertAddress(int pnseed) throws UnknownHostException{
		byte[] v4addr = new byte[4];
		v4addr[0]=(byte)(0xFF&(pnseed));
		v4addr[1]=(byte)(0xFF&(pnseed>>8));
		v4addr[2]=(byte)(0xFF&(pnseed>>16));
		v4addr[3]=(byte)(0xFF&(pnseed>>24));
	...
	public static interface SinglePeerAddressDiscovery
